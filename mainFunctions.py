...
def data_preview():
    return data.head()
    #Wow so you can use DataFrame commands into a custome function
    #requires line 3 import
...
def slug_split():
    ...
    return data
    #Ex. Access the column "slug" returns GBP/EGP. Split at the /
    #for a in each row,
        #data["A"] = Split at / and take the index 0. Store it as str(a)
        #data["B"] = Split at / and take the index 1. Store it as str(a)
        # A and B are newly created columns. This is how it's done in Pandas
...
"""
def uniq_cur():
    # data = slug_split() #Does this line even matter? No errors when this is removed anyway
    curA = data["A"].unique()
    curB = data['B'].unique()
    return curA, curB
...
close of a currency pair(exchange) 


If Ohlc is not set to True, the function will return the first five row of    
the data frame. 

Because of the high number of unique currencies on the 
...
#currencyA = 'GBP' 
# cur = str('GBP')
#from data, select column A and retrieve only rows cur that is = 'GBP'
    #essentially "another dataset" called curA
#set the index to GET datetime values in the date column#

#Can't visualize it starting from here.
#get all unique slugs in curA, converts them into a list called xch
#for all values of x  in xch
    #
...
def aut_arima(train_data):
    ...
    #Produces details such as coeff, std error
    print(model_autoARIMA.summary())
    model_autoARIMA.plot_diagnostics(figsize=(15,8))
    
    #retrieves the values of p,d,q and orders them
    global p, d, q
...
"""
A_MAPE = 0 #Arima Mean Absolute Percentage Error
P_MAPE = 0 #Prophet Mean Absolute Percentage Error


#Separate call -- NOT LINKED WITH aut_arima
...
def arima(train_data, test_data, plot=False):
    ...
    fc=fitted.forecast(samples, alpha=0.05) #will generate a forecast for the next n time periods.
    fc_series = pd.Series(fc, index=test_data.index) #creates a 
    ...
    if plot == True:
        ...
        mse = mean_squared_error(test_data, fc) #takes the (past: test data, predicted: fc)
        print('MSE: '+str(mse))
...
def prophet_model(train_data, test_data, plot=False):
    ...
    train_df["ds"]=train_df.index #the index which is datetime is stored in train_df as the column "ds"
    train_df["y"]=train_df["close"]
    model = Prophet(seasonality_mode='additive', 
                    weekly_seasonality=True, daily_seasonality=True) #annotate later
    model.fit(train_df)
    
    
    future = model.make_future_dataframe(periods=len(test_data), freq='W-SUN',include_history=False) #annotate later
